...
class OpossumPulseGenerator {

    companion object {
        const val TAG = "OpossumPulseGenerator"
    ...
    private fun buildB0Command(pulses: List<Pulse>): ByteArray {
        ...
        for (i in 0 until 4) {
            ampA[i] = (pulses[i].ampA * channelAAmp / 200.0f).toInt().coerceIn(0, 100).toByte()
            ampB[i] = (pulses[i].ampB * channelBAmp / 200.0f).toInt().coerceIn(0, 100).toByte()
        }
        return byteArrayOf(
            0xB0.toByte(),
